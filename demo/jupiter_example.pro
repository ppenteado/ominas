...
;
;     limb_ptd = pg_limb(gd=gd) & pg__hide, limb_ptd, gd=gd, bx=rd, /rm
;               pg__hide, limb_ptd, /assoc, gd=gd, bx=pd, od=sund
;     ring_ptd = pg_disk(gd=gd) & pg__hide, ring_ptd, gd=gd, bx=pd
;     term_ptd = pg_limb(gd=gd, od=gd.sund) & pg__hide, term_ptd, gd=gd, bx=pd, /assoc
;
...
limb_ptd = pg_limb(gd=gd) & pg__hide, limb_ptd, gd=gdbx=rd, /rm
       pg__hide, limb_ptd, /assoc, gd=gd, bx=pd, od=sund
ring_ptd = pg_disk(gd=gd) & pg__hide, ring_ptd, gd=gd, bx=pd
center_ptd = pg_center(gd=gd, bx=pd)
term_ptd = pg_limb(gd=gd, od=gd.sund) & pg__hide, term_ptd, gd=gd, bx=pd, /assoc
...
;
;     limb_ptd = pg_limb(gd=gd) & pg__hide, limb_ptd, gd=gd, bx=rd, /rm
;             pg__hide, limb_ptd, bx=pd, /assoc, gd=gd, od=sund
;     ring_ptd = pg_disk(gd=gd) & pg__hide, ring_ptd, gd=gd, bx=pd
;     center_ptd = pg_center(gd=gd, bx=pd)
;     term_ptd = pg_limb(gd=gd, od=gd.sund) & pg__hide, term_ptd, gd=gd, bx=pd, /assoc
;     object_ptd = [center_ptd,limb_ptd,ring_ptd,term_ptd]
...
;   It uses pg__hide to set as not visible the points on the grid behind the
...
plat_ptd = pg_grid(gd=gd, slon=!dpi/2d, lat=lat, nlon=0) 
pg__hide, plat_ptd[0], cd=cd, bx=pd[0], /assoc
pg_draw, plat_ptd[0], psym=3, plabel=strtrim(round(lat*180d/!dpi),2), /label_p
...
dgrid_ptd=pg_grid(gd=gd, bx=rd, frame_bd=pd[0]) & pg__hide, dgrid_ptd, gd=gd, bx=pd
pg_draw, dgrid_ptd, color=ctpurple()
