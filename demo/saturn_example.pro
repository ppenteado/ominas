...
;  to these routines.  It also uses pg__hide to remove (/rm) points from the
...
limb_ptd = pg_limb(gd=gd) & pg__hide, limb_ptd, gd=gd, bx=rd
...
grid_ptd = pg_grid(gd=gd, bx=pd) 
   pg__hide, grid_ptd, gd=gd, bx=pd, /assoc
...
ring_ptd = pg_disk(gd=gd) & pg__hide, ring_ptd, gd=gd, bx=pd, /rm
shadow_ptd = pg_shadow_globe(gd=gd, ring_ptd)
center_ptd = pg_center(gd=gd, bx=pd)
object_ptd = [center_ptd,limb_ptd,ring_ptd,shadow_ptd]
