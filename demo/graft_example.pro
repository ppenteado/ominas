...
; and can figure out what all those things are all supposed to be, so limbs
; get are limbs, rings are rings, etc.  GRAFT also surreptitiously sneaks
...
; any kind of geometric object, so you have to use the right button on 
; them.  
;
; When you have had enough of grafting and blasting edge points for no
; reason, let's move on to the farfit.  PG_FARFIT is a quick-and-crappy 
; fit of the limb points (just the zero-th ones) to the edge points,
; which may or may not be currently displayed by GRIM at this point.  
; It returns an x/y offset that can be used to derive a correction to
; the orientation of the camera descriptor cd.  PG_REPOINT is used to 
; apply the pointing correction.  Note that GRIM acknowledges this outrage
; by recomputing all of the overlay points based on the modified camera
; descriptor.
;
;-------------------------------------------------------------------------
