...
; OBSERVATION TIMES
;
;  Here we set the start and stop times.  For readability, we'll use UTC, 
;  making this step inherently translator-dependent, as that conversion is
;  handled in the SPICE translator package, assuming you have it installed.
;  You could avoid the direct calls to subroutines inside the SPICE 
;  translator system by directly assigning t_start and t_stop using explicit 
;  ephemeris times, but that would still ultimately depend on the translator
;  setup since OMINAS does not impose any unit systems or time systems:: 
;
...
;-------------------------------------------------------------------------
times = ['2017-08-21T19:00:00','2017-08-21T20:00:00']
pd = pg_get_planets(time=times)



pd = pg_get_planets(time=['2017-08-21T19:00:00','2017-08-21T20:00:00'])
cd = pg_get_cameras(time=['2017-08-21T19:00:00','2017-08-21T20:00:00'])
