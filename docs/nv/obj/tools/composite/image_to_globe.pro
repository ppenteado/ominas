 ...
 case class of 
  ...
  'CAMERA' : $
	begin
	 ...
	 v = bod_inertial_to_body_pos(od, cam_pos)

	 r_inertial = (bod_body_to_inertial(cd, $
	                cam_focal_to_body(cd, $
	                  cam_image_to_focal(cd, p))))[linegen3z(np,3,nt)]
	 r = bod_inertial_to_body(od, r_inertial)

	 body_pts = glb_intersect(od, v, r, discriminant=discriminant)
         valid = where(discriminant GE 0)

         w = where(discriminant GE 0)
         if(w[0] EQ -1) then return, 0

	 return, glb_body_to_globe(od, body_pts)
	end
