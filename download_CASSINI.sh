...
for dir in "${dirs[@]}"
do
 ./pp_wget "${baseurl}${dir}/ --localdir=${1}/$dir/ --absolute --timestamps=$ts --lm=1 $@"
done

#special treatment directories (spk and ck, which are large)
echo "Downloading spks"
./pp_wget "${baseurl}spk/ --localdir=${1}/spk/ $@ --absolute --timestamps=$ts --lm=1" # --xpattern=(\.bsp$)|(\.bsp\.lbl$)"
...
./pp_wget "${baseurl}ck/ --localdir=${1}/ck/ $@ --absolute --timestamps=$ts --lm=1 --pattern="\
"(([[:digit:]]{5}_[[:digit:]]{5}(r[[:alnum:]])|([[:alnum:]]{2}_ISS))|(17[[:digit:]]{3}_[[:digit:]]{5}[[:alnum:]]{2}_[fp]siv))(\.bc$)|(bc\.lbl$)"
