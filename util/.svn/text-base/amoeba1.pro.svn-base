; $Id: amoeba.pro,v 1.8 1999/05/28 16:37:21 ali Exp $
; Copyright (c) 1996-1999, Research Systems, Inc.  All rights reserved.
;	Unauthorized reproduction prohibited.


Function amotry, p, y, psum, func, ihi, fac, term=term
...
; point is better.
  ...
  ytry = call_function(func, ptry, term=term)  ;Eval fcn at trial point
...
Function Amoeba1, ftol, FUNCTION_NAME=func, FUNCTION_VALUE=y, $
...
while ((ncalls le nmax) AND NOT done) do begin		;Each iteration
...
;while ncalls le nmax do begin		;Each iteration
  ...
  ncalls = ncalls + 2
  ytry = amotry(p, y, psum, func, ihi, -1.0, term=term)
  if ytry le y[ilo] then ytry = amotry(p,y,psum, func, ihi, 2.0, term=term) $
  else if ytry ge y[inhi] then begin
    ysave = y[ihi]
    ytry = amotry(p,y,psum,func, ihi, 0.5, term=term)
    if ytry ge ysave then begin
	for i=0, ndim do if i ne ilo then begin
	  psum = 0.5 * (p[*,i] + p[*,ilo])
	  p[*,i] = psum
	  y[i] = call_function(func, psum, term=term)
	  endif
	ncalls = ncalls + ndim
	psum = total(p,2)
	endif		;ytry ge ysave
    endif else ncalls = ncalls  - 1
    if(keyword_set(term)) then done = 1
  endwhile
