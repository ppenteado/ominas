; $Id: //depot/idl/IDL_70/idldir/lib/cw_animate.pro#1 $
...
FUNCTION CW__ANIMATE_EV, event
  ...
  compile_opt hidden
   ...
   CASE event.id OF
      ...
      state.wPNGButton : BEGIN
         pngFilename=DIALOG_PICKFILE(/WRITE)
         IF STRLEN(pngFilename) GT 0 THEN BEGIN
            tmpBase=WIDGET_BASE(TITLE='Animation: Creating PNG files', $
                       /FLOATING, MODAL=WIDGET_INFO(event.top, /MODAL), $
                       GROUP=event.top, TLB_FRAME_ATTR=27, /FRAME)
            tmpMsg = 'Writing PNG data:'
            tmpText=WIDGET_TEXT(tmpBase, $
               XSIZE= ( STRLEN(tmpMsg) > STRLEN(pngFilename) )+5, $
               YSIZE=4, $
               /ALIGN_CENTER, $
               VALUE=['', tmpMsg, pngFilename])
           ...
           ; Spitale -----------------
             ...
             split_filename, pngFilename, dir, name, ext
             if(NOT keyword_set(dir)) then dir = '.' 
             if(NOT keyword_set(ext)) then ext = 'png'

             for i=0, n_elements((*state.pwinHdl))-1 do $ 
              begin
               if((*state.pwinHdl)[i] GT -1) then $
                begin
                 wset, (*state.pwinHdl)[i]
                 image= tvrd()
                 fname = dir + '/' + name + '_' + $
                       str_pad(strtrim(i,2), 3, align=1, c='0') + '.' + ext
                 print, 'writing ' + fname
                 write_png, fname, image, order=order
                end
              end
           ...
           ENDIF ELSE BEGIN
               XDISPLAYFILE, "animatedemo.hlp", TITLE = "Animation Help", $
                 ...
                 TEXT = [ $
                  ...
                  "      The animation may saved as PNG files by", $
                  "selecting the 'Write PNG' button.  Note that this", $
      ...
      state.wOpenButton : BEGIN
         ...
         CW__ANIMATE_RUN, wAnimateBase, /STOP
         ...
         IF fileOK THEN BEGIN
            ...
            IF framedelta EQ 0 THEN $
               framedelta = 1
            CW__ANIMATE_RUN, wAnimateBase, rate, DELTA=framedelta, /LASTFRAME
         ENDIF ELSE $
              ; Disable all controls until all frames are loaded
...
pro CW__ANIMATE_LOAD, widget, IMAGE = image, FRAME = frame, ORDER = order, $
   ...
   IF N_ELEMENTS(image) NE 0  THEN BEGIN            ;Make sure image was set.
      ...
      WSET, old_window
      GOTO, Done			; Spitale -- are you f-ing kidding me?  Who the f wrote this???
...
END ;CW__ANIMATE_LOAD




pro CW__ANIMATE_RUN, widget, rate, STOP = stop, NFRAMES = nframes, $
   ...
   WIDGET_CONTROL, widget, TIMER=state.delay, EVENT_FUNC = 'CW__ANIMATE_EV'
 done:
...
pro CW__ANIMATE_GETP, widget, PIXMAPS, KILL_ANYWAY = kill_anyway
...
PRO CW__ANIMATE_INIT, wAnimateBase, sizex, sizey, nframes, PIXMAPS=old_pixmaps
...
function CW__ANIMATE, parent, sizex, sizey, nframes, $
    UVALUE=uval, $
  ...
  COMMON BitmapButtons
   ...
   ELSE $
      wOpenButton = 0
   wPNGButton = WIDGET_BUTTON(wButtonBase, VALUE='Write PNG', $
       UNAME=uname+'_PNG')
   ...
   WIDGET_CONTROL, wAnimateBase, SET_UVALUE = uval, EVENT_FUNC = 'CW__ANIMATE_EV', $
       /DELAY_DESTROY
   ...
   WIDGET_CONTROL, wTopBase, SET_UVALUE = $
      { wEndAnimationButton: wEndAnimationButton, $      ; End button
        ...
        wPNGButton: wPNGButton, $                      ; PNG button
  ...
  ; procedure called to mop up
   WIDGET_CONTROL, wTopBase, KILL_NOTIFY = 'CW__ANIMATE_CLN'

   CW__ANIMATE_INIT, wAnimateBase, sizex, sizey, nframes, PIXMAPS=old_pixmaps
