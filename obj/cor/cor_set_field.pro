...
;
;===========================================================================
pro cor_set_field, odp, tag_name, value, found=found, noevent=noevent
 ods = cor_dereference(odp)
 ...
 while (size(_ods, /type) EQ 8) do $
  begin
   ...
   for i=0, ntags-1 do if(tags[i] EQ strupcase(tag_name)) then $
    begin
     _ods.(i) = value
     cor_rereference, _odp, _ods
     nv_notify, _odp, type = 0, noevent=noevent
     return
    end
   _odp = _ods.(0)
   if(obj_valid(_odp)) then _ods = cor_dereference(_odp)
  end 
