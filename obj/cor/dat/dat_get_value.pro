...
;	and a warning is issued.  This behavior is disabled if $OMINAS_DEBUG is 
;	set (e.g., +OMINAS_DEBUG=1 or +OM_DEBUG=1 from the shell prompt).
;
;
; CATEGORY:
;	NV/SYS
...
; need to group dd based on instrument...
 ...
 nv_suspend_events
 nv_message, verb=0.8, 'Data descriptors ' + str_comma_list(cor_name(dd))
 nv_message, verb=0.8, 'Keyword ' + keyword
...
@dat_trs_keywords1_include.pro
   ...
   else nv_message, verb=0.8, 'No value.' 
  end 
 nxds = n_elements(xds)
 ...
 nv_message, verb=0.8, 'Output descriptors: ' + str_comma_list([cor_name(result)])
