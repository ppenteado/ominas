...
;				OMINAS_TRANSLATOR_TABLE.  If no path is given, 
;				then the file is assumed to reside in the same 
;				directory as the translator named by the 
;				OMINAS_TRANSLATOR_TABLE environment variable.
;
...
;	OMINAS_TRANSLATOR_TABLE:Name(s) of the translators table(s) to use unless 
;				overridden by the tab_translators keyword.
;				Multiple table names are delimited with ':'.
;
;	OMINAS_FILETYPE_TABLE:	Name(s) of the filetype detectors table(s).		
;				Multiple table names are delimited with ':'.
;
;	OMINAS_IO_TABLE:	Name(s) of the I/O table(s).		
;				Multiple table names are delimited with ':'.
;
;	OMINAS_INSTRUMENT_TABLE:Name(s) of the instrument detectors table(s).			
;				Multiple table names are delimited with ':'.
;
;
; RETURN:
  ...
  begin
   ...
   if(NOT keyword_set(_instrument)) then  $
    begin
     ...
     nv_message, verb=0.8, 'Instrument = ' + instrument
    end $
...
function drd_match_extensions, filename, extensions, all=all
 ...
 if(keyword_set(all)) then $
  begin
   ...
   void = str_nnsplit(base, delim, rem=ext)
   return, delim + ext
  end
