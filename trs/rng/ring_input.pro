...
;	OMINAS_RNG_DATA:	Sets the directory in which to look for data files.
;
;
;  TRANSLATOR KEYWORDS:
...
;
;=============================================================================
pro ri_system, pd, dkd, inner=inner, outer=outer
 ...
 cor_set_name, dkd, 'MAIN_RING_SYSTEM' + '-' + cor_name(pd)
...
@dat_trs_keywords1_include.pro
 ...
 ; get catalog path
 ;----------------------------------------------
 catpath = getenvs('OMINAS_TRS_RNG_DATA')
 if(NOT keyword_set(catpath)) then $
   begin
    nv_message, /con, $
     'OMINAS_TRS_RNG_DATA environment variable is undefined.', $
       exp=['OMINAS_TRS_RNG_DATA specifies directories in which this translator', $
    ...
    if(keyword_set(dat)) then $
      begin
       ...
       if(keyword_set(system)) then $
        begin
         ri_system, pds[i], dkd_inner, /inner
         ri_system, pds[i], dkd_outer, /outer
        end
