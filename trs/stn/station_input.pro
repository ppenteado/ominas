...
;	OMINAS_STN_DATA:	Sets the directory in which to look for data 
;				files.
;
;
; TRANSLATOR KEYWORDS:
...
@dat_trs_keywords1_include.pro
 ...
 ;----------------------------------------------
 catpath = getenvs('OMINAS_TRS_STN_DATA')
; catpath = nv_getenv(/data)
 if(NOT keyword_set(catpath)) then $
  begin
   nv_message, /con, $
     'OMINAS_TRS_STN_DATA environment variable is undefined.'
       exp=['OMINAS_TRS_STN_DATA specifies the directory under which this translator', $
   ...
   if(keyword_set(dat)) then $
    begin
     ...
     if(continue) then $
      begin
       ndat = n_elements(dat)
       _stds = stn_create_descriptors(ndat, gd=make_array(ndat, val=cor_create_gd(cor_gd(xd[i]), bx0=xd[i], dd=dd)))
