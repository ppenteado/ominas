...
;
;===========================================================================
function cas_cirs_spice_time, dd, dt=dt, string=string, status=status, exposure=exposure

 status = -1
 label=dat_header(dd)
 start_time=min(label.table.scet,max=close_time)
 ...
 exposure*=86400d0
 dt = -0.5d*exposure
 nv_message,verb=0.91,'CIRS START_TIME='+strtrim(start_time,2)
