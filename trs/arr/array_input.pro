...
;	OMINAS_ARR_DATA:	Sets directory in which to look for data files.
;
;
; TRANSLATOR KEYWORDS:
;	NONE
...
@dat_trs_keywords1_include.pro
 ...
 ; check array catalog
 ;----------------------------------------------
 catpath = getenvs('OMINAS_TRS_ARR_DATA')
 if(NOT keyword_set(catpath)) then $
  begin
   nv_message, /con, $
     'OMINAS_TRS_ARR_DATA environment variable is undefined.', $
       exp=['OMINAS_TRS_ARR_DATA specifies the directory under which this translator', $
      ...
      begin
       ...
       if(keyword_set(dat)) then $
        begin
         ...
         ;- - - - - - - - - - - - - - - - - - - - - - - -
         _ards = arr_create_descriptors(gd=cor_create_gd(cor_gd(xd[i]), bx0=xd[i], dd=dd))
