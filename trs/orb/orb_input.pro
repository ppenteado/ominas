...
;	OMINAS_ORB_DATA:	Sets the directory in which to look for data files.
;
;
; RETURN:
...
@dat_trs_keywords1_include.pro
 ...
 ; get catalog path
 ;----------------------------------------------
 catpath = getenvs('OMINAS_TRS_ORB_DATA')
 if(NOT keyword_set(catpath)) then $
  begin
    nv_message, /con, $
     'OMINAS_TRS_ORB_DATA environment variable is undefined.', $
       exp=['OMINAS_TRS_ORB_DATA specifies directories in which this translator', $
