...
function ucac4_get_stars, dd, filename, parm
 ...
 for i = 0, nz do $
  begin
   ...
   openr, zone, getenv('OMINAS_TRS_STRCAT_UCAC4_DATA') + sep + z_fname, /get_lun
   point_lun, zone, first * rec_bytes
   nv_message, verb=0.9, 'Reading ' + string(z_strs) + ' stars from zone file ' + getenv('OMINAS_TRS_STRCAT_UCAC4_DATA') + sep + z_fname 
   readu, zone, z_recs
