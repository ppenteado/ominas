...
;                    filters = ['LBLTIME', 'SEGLEN', 'ITIME', 'MTIME']
 ...
 if(ticks) then $
  if(NOT keyword_set(sc)) then $
   begin
    nv_message, verb=0.9, 'Aborting because spacecraft must be specified.'
    return, ''
   end
...
; if((NOT keyword_set(all)) AND  (NOT keyword_set(time))) then $
 ...
 ss = sort(select_time)
;print
;if(type EQ 'sp') then print, caller(/all)
;if(type EQ 'sp') then print, files[ss]
 return, files[ss]
end
;=============================================================================
