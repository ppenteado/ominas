...
FUNCTION pp_label2hash_proc,lbl,i
...
REPEAT BEGIN
	...
	IF (pos[0] EQ -1) THEN BEGIN
		PRINT,'pp_label2hash: no "=" on line ',i
		STOP
	ENDIF
	...
	IF (key EQ 'OBJECT') THEN BEGIN
	   ...
	   ;Result = EXECUTE(value + ' = pp_label2hash_proc(lbl,i)')
	   (scope_varfetch(value,/enter))=pp_label2hash_proc(lbl,i)
	   ...
	   IF (N_ELEMENTS(keys_n) EQ 0 || WHERE(keys_n EQ value) EQ -1) THEN BEGIN
		...
		(scope_varfetch(value+'LIST',/enter))=list(scope_varfetch(value))
	   ...
	   ENDIF ELSE BEGIN
		...
		(scope_varfetch(value+'LIST',/enter)).add,scope_varfetch(value)
	   ENDELSE
	   key = value + 'LIST'
	   ;Result = EXECUTE('hh["' +value+'LIST" ] = '+value+'LIST')
	   hh[value+'LIST']=scope_varfetch(value+'LIST')
...
FUNCTION pp_label2hash,lblfile
...
ENDFOR
hh = pp_label2hash_proc(lbllist,0)
RETURN,hh
END
