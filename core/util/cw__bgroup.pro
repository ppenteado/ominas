; $Id: //depot/Release/ENVI50_IDL82/idl/idldir/lib/cw_bgroup.pro#2 $
;
...
;   2/2021, Spitale; fixed bug where first exclusive button was indicated as set
;      when no setting had been made.
;-


pro CW__BGROUP_SETV, id, value
  compile_opt hidden
...
function CW__BGROUP_GETV, id, value
...
function CW__BGROUP, parent, names, $
  ...
  IF (N_ELEMENTS(uname) eq 0)      then uname = 'CW__BGROUP_UNAME'
  ...
  WIDGET_CONTROL, top_base, MAP=map, $
    FUNC_GET_VALUE='CW__BGROUP_GETV', PRO_SET_VALUE='CW__BGROUP_SETV', $
  ...
  if(excl) then widget_control, ids[0], /set_button		; Spitale

  if (n_elements(sval) ne 0) then CW__BGROUP_SETV, top_base, sval
