...
; need to free rdt, gbxt if not returned!!!
 ...
 if(arg_present(inc)) then inc = orb_get_inc(rdt, gbxt)
 if(arg_present(ap) $
    OR arg_present(lp)) then ap = orb_get_ap(rdt, gbxt)
 if(arg_present(lan)) then lan = orb_get_lan(rdt, gbxt)
 if(arg_present(ma) $
    OR arg_present(ml)) then ma = orb_get_ma(rdt)
 if(arg_present(dapdt) $
    OR arg_present(dlpdt) $
    OR arg_present(dmldt)) then dapdt = orb_get_dapdt(rdt, gbxt)
 if(arg_present(liba_ap)) then liba_ap = orb_get_liba_ap(rdt, gbxt)
 if(arg_present(dlibdt_ap)) then dlibdt_ap = orb_get_dlibdt_ap(rdt, gbxt)
 if(arg_present(lib_ap)) then lib_ap = orb_get_lib_ap(rdt, gbxt)
 if(arg_present(dlandt) $
    OR arg_present(dlpdt) $
    OR arg_present(dmldt)) then dlandt = orb_get_dlandt(rdt, gbxt)

 if(arg_present(liba_lan)) then liba_lan = orb_get_liba_lan(rdt, gbxt)
 if(arg_present(dlibdt_lan)) then dlibdt_lan = orb_get_dlibdt_lan(rdt, gbxt)
 if(arg_present(lib_lan)) then lib_lan = orb_get_lib_lan(rdt, gbxt)

 if(arg_present(dmadt) $
    OR arg_present(dmldt)) then dmadt = orb_get_dmadt(rdt)

 if(arg_present(lp)) then lp = orb_arg_to_lon(rdt, ap, gbxt)
 if(arg_present(ml)) then ml = orb_anom_to_lon(rdt, ma, gbxt)
 ...
 if(arg_present(tl)) then tl = reduce_angle(orb_anom_to_lon(rdt, ta, gbxt)) ; there may be a problem with this.
