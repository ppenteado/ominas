...
;		is obtained from the OMINAS_DAT_MAP_DATA environment varable.
;
;  OUTPUT: NONE
;
;
...
common pg_load_maps_block, dd_cache, md_cache
 ...
 ; get map directory
 ;--------------------------------------------------------------
 if(NOT keyword_set(mapdir)) then mapdir = getenv('OMINAS_DAT_MAP_DATA')
  ...
  begin
   nv_message, /con, 'Map directory not specified.', $
       exp=['The map directory may be specified as the argument to this program', $
            'or via the OMINAS_DAT_MAP_DATA environment variable.']
   return, 0
  end 
