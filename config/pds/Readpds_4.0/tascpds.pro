...
function CONVERT_TABLE_ELEMENT, element, type, rows
    ...
    if (type NE "N/A") then begin
        if (stat[0] EQ 1 AND stat[1] GT 1 AND rows GT 1) then begin
            temp = strarr(stat[1])
...
function CREATE_TABLE_STRUCT, keywds, opt, items, label, start_ind, end_ind
    ...
    ; if cursor position < rb-2 then take the difference between 78 and curpos
    ; and create a temporary buffer to be read at the end of each record.
    ; if cursor position > rb-2 then issue error, and exit routine:
    if (keywds.row_bytes - 2 GT curpos) then begin
        diff = keywds.row_bytes - 2 - curpos
        data_set = create_struct(data_set, "temp", bytarr(diff))
    endif else if (keywds.row_bytes - 2 LT curpos) then begin
        ; check for row suffix bytes:
        if (opt.suffix GT 0) then begin
            data_set = create_struct(data_set,"suffix", bytarr(opt.suffix-2))
        endif else begin
    ...
    ; set the CR and LF structure elements:
...
;-----------------------------------------------------------------------------
; precondition: data contains the data from the file in a structure.
; postcondition: the cr and lf characters are checked for their presence, 
;     and any inconsistency returns -1, else 1.
;

function CHECK_TABLE_CRLF, data
   ; find the positions where the cr and lf chars are present in struct:
   crpos = where (data.crlf EQ 13B, crcount)
   lfpos = where (data.crlf EQ 10B, lfcount)
   ...
   if ((crcount NE n_elements(data)) AND $
       (lfcount NE n_elements(data))) then begin
...
function ORGANIZE_TABLE_DATA, keywds, data
    ...
    for i = 0, keywds.columns - 1 do begin
        ...
        data_struct = create_struct(data_struct, title, element)
    endfor
...
function TASCPDS, fname, label, objindex, SILENT=silent
    ...
    if (TASC_INTERFORM(label, start_ind, end_ind) EQ -1) then begin
        return, -1
    endif
   ...
   ; check for CR and LF chars using subroutine:
    if (CHECK_TABLE_CRLF(data) EQ -1) then begin
        return, -1
    endif

    ; separate data into columns and convert into appropriate type:
    data_set = ORGANIZE_TABLE_DATA (req_keywds, data)          ; subroutine
    if (data_set.flag EQ 1) then begin
        data = data_set.data
    endif else begin
        return, -1
