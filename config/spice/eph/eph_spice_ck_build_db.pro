...
;+
; NAME:
;       eph_spice_ck_build_db
;
;
; PURPOSE:
;       Read/Build the SPICE kernel CK database
;
;
; CATEGORY:
;       UTIL/SPICE
...
;
; CALLING SEQUENCE:
;       eph_spice_ck_build_db, ckpath, data 
;
;
; ARGUMENTS:
;  INPUT:
...
;			first:		Range start (sclk ticks)
;			last:		Range end (sclk ticks)
...
;			stillThere:	File in database still exists in ckpath
;
;  OUTPUT: NONE
;
;
...
;			Filename is spice_ck_database.<ckpath with / replaced by _ >
;			Compares/generates list of filename in ckpath
;			If file system time matches current database, uses that information
;                       Uses spice to calculate if it is a CK, and the range if so
...
;-
;=============================================================================
pro eph_spice_ck_build_db, ckpath, data, debug=debug
 ...
 ; replace / with _
 fix_path = strjoin(strsplit(ckpath,'/',/extract),'_')
 ; replace * with x
 fix_path = strjoin(strsplit(fix_path,'*',/extract,/preserve_null), 'x')
 dbfile = '~/.ominas/spice_ck_database.' + fix_path
 ...
 while ~EOF(unit) do begin
   ...
   if debug_set then begin
        print, format='(A,A,I,A,I,A,I11,A,F15.3,A,F15.3)', db.filename[count],',',db.first[count],',', $
 ...
 ; get all file names
 ;------------------------
 all_files = file_search(ckpath + '/*')

 ;-----------------------------------
 ; loop over all files to check if ck
 ;-----------------------------------
 ...
 for i=0, newcnt-1 do begin
   ...
   if error ne 0 then begin
     ...
     if debug_set then print, 'Not a ck file: ' + all_files[i] + ' (Error: ' + !error_state.name + ')'
     goto, addToOutput
   endif
   ...
   if ts_tried EQ 0 then begin
     eph_spice_read_timestamps, ckpath, unit, ts_filenames, timestamps, debug=debug
     ts_tried = 1
   ...
   cspice_ckobj, all_files[i], ids
   nobjs = cspice_card(ids)
   if nobjs EQ 0 then goto, addToOutput
   if debug_set then print, 'CK file: ' + all_files[i] + ' contains: '
   for j=0, nobjs-1 do begin
     ...
     cspice_ckcov, all_files[i], objnum, SPICEFALSE, 'SEGMENT', 0.D, 'SCLK', cover
   ...
   addToOutput:
   line = string( format='(A,A,I,A,I,A,I11,A,F15.3,A,F15.3)', all_files[i],',',min_first,',',max_last,',', $
 ...
 ; Test to see if any original ck files in database will be removed
 if exists EQ 1 then begin
