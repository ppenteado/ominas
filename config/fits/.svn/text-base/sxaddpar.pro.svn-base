...
if keyword__set( BEFORE) then loc = strupcase(before) else $
if keyword__set( AFTER)  then loc = strupcase(after) else $
                             loc = 'END'
...
if loc ne '' then begin
          ...
          if nloc gt 0 then begin
             i = iloc(0)
             if keyword__set(after) and loc ne 'HISTORY ' then i = i+1 < iend 
