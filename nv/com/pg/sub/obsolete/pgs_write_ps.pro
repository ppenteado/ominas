...
;-
;=============================================================================
pro pgs_write_ps, filename, ps, bin=bin
nv_message, /con, name='pgs_write_ps', 'This routine is obsolete.'
  ...
  begin
   ...
   if(ptr_valid(ps[i].data_p)) then $
    begin
     ...
     if(ndim GT 2) then nv_message, name='pgs_write_ps', $
                           'Point data may have no more than 2 dimensions.'
     s = s[1:s[0]]
     w = where(s EQ n)
     if(w[0] EQ -1) then nv_message, name='pgs_write_ps', $
