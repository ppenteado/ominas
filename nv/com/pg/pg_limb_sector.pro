...
;-
;=============================================================================
function pg_limb_sector, cd=cd, gbx=_gbx, gd=gd, $
 ...
 ; initial point
 ;-----------------------------------
 if(NOT keyword__set(p0)) then cursor, px, py, /down $
 else $
  begin
   px = p0[0] & py = p0[1]
  end
  ...
  if(button EQ 0) then done = 1 $
   ...
   begin
    ...
    outline_pts = get_limb_profile_outline(cd, gbx, dkd=dkd, $
                        tr([tr(point0), tr(point)]), nalt=nalt, naz=naz, save=save)
 ...
 outline_pts = get_limb_profile_outline(cd, gbx, $
    tr([tr(point0), tr(point)]), nalt=nalt, naz=naz, limb=limb_pts_body, $
                                     scan_alt=altitides, scan_az=azimuths)
