...
;
;=============================================================================
function ps_read_0, filename, visible=visible, $
 ...
 for i=0, nps-1 do $
  begin
   ...
   ps = append_array(ps, ps_init(name=name[i], desc=_desc, flags=_flags, $
...
;
;=============================================================================
function psrps_get_next, unit, token, stop=stop, status=status, n=n, $
                           bin=bin, buf=buf
...
;
;=============================================================================
function ps_read_1, filename, visible=visible, $
 ...
 repeat $
  begin
   ...
   ; name, desc, n
   ;- - - - - - - - - - - - - - - - - - -
   line = psrps_get_next(unit, name_token, stat=stat)
   if(stat EQ -1) then done = 1 $
   else $
    begin
     name = strtrim(strep_s(line, name_token, ''), 2)
     desc = strtrim(strep_s(psrps_get_next(unit), 'desc =', ''), 2)
     if(version GT 0) then input = strtrim(strep_s(psrps_get_next(unit), 'input =', ''), 2)
     n = fix(strtrim((str_sep(psrps_get_next(unit), ' '))[2], 2))

     ;- - - - - - - - - - - - - - - - - - -
     ; points
     ;- - - - - - - - - - - - - - - - - - -
     line = psrps_get_next(unit, 'points:', stop=':', stat=stat)
     if(stat EQ 0) then $
      begin
       buf = psrps_get_next(unit, n=n, bin=bin, buf=dblarr(2,n))
       if(bin) then p = buf $
       else $
        begin
     ...
     ; vectors
     ;- - - - - - - - - - - - - - - - - - -
     line = psrps_get_next(unit, 'vectors:', stop=':', stat=stat)
     v = 0
     if(stat EQ 0) then $
      begin
       buf = psrps_get_next(unit, n=n, bin=bin, buf=dblarr(3,n))
       if(bin) then v = buf $
       else $
        begin
     ...
     ; point data
     ;- - - - - - - - - - - - - - - - -
     line = psrps_get_next(unit, 'point data:', stop=':', stat=stat)
     if(stat EQ 0) then $
      begin
       ...
       ;- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        begin
         ...
         buf = psrps_get_next(unit, n=n, bin=bin, buf=dblarr(nn,n))
...
;           for i=0, nn-1 do data[*,i] = str_nnsplit(rem, ' ', rem=rem)
     ...
     ; generic user data
     ;- - - - - - - - - - - - - - - - -
     line = psrps_get_next(unit, 'udata:', stop=':', stat=stat)
     ...
     ; tags
     ;- - - - - - - - - - - - - - - - - - -
     line = psrps_get_next(unit, 'tags:', stop=':', stat=stat)
     if(stat EQ 0) then $
          tags = psrps_get_next(unit, n=nn, buf=bytarr(nn))


     ;- - - - - - - - - - - - - - - - - - -
     ; flags
     ;- - - - - - - - - - - - - - - - - - -
     line = psrps_get_next(unit, 'flags:', stop=':', stat=stat)
     if(stat EQ 0) then $
      begin
       buf = psrps_get_next(unit, n=n, bin=bin, buf=bytarr(n))
       if(bin) then f = buf $
       else f = byte(fix(buf))
      end
     ...
     ;- - - - - - - - - - - - - - - - - - -
     ps = append_array(ps, $
             ps_init(name=name, desc=desc, flags=f, $
...
;
;=============================================================================
function ps_read, filename, bin=bin, $
         name=name, desc=desc, flags=flags, points=points, vectors=vectors, $
...
@ps_include.pro
 ...
 if(keyword_set(s[0])) then $
  begin
   if(s[0] NE 'protocol') then nv_message, name='ps_read', $
                                 'Syntax error in file ' + filename+ '.'
   w = str_isalpha(s[1])
   if(w[0] NE -1) then nv_message, name='ps_read', $
                                 'Syntax error in file ' + filename+ '.'
   protocol = fix(s[1])
 ...
 case protocol of
  0: begin
      ps = ps_read_0(filename, visible=visible, $
         name=name, desc=desc, flags=flags, points=points, vectors=vectors, $
         comment=comment)
     end
  1: ps = ps_read_1(filename, visible=visible, $
         name=name, desc=desc, flags=flags, points=points, vectors=vectors, $
         comment=comment, version=version, input=input, data=data, tags=tags)
  else: nv_message, name='ps_read', 'Invalid protocol.'
 endcase
