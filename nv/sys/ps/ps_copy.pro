...
;-
;==============================================================================
pro ps_copy, ps_dst, ps_src

 if(NOT keyword_set(ps_src)) then return

 for i=0, n_elements(ps_src)-1 do $
  begin
   ps_get, ps_src[i], p=p, vec=v, f=f, tags=tags, data=data, $
                          name=name, desc=desc, input=input, assoc_idp=assoc_idp
   ps_set, ps_dst[i], p=p, vec=v, f=f, tags=tags, data=data, $
                          name=name, desc=desc, input=input, assoc_idp=assoc_idp

   tag_list_copy, ps_udata(ps_dst[i]), ps_udata(ps_src[i])
  end
...
;==============================================================================
pro __ps_copy, ps_dst, ps_src

 if(NOT keyword_set(ps_src)) then return

 for i=0, n_elements(ps_src)-1 do $
  begin
   nv_notify, ps_src[i], typ=1, desc='ALL'
   ...
   ;--------------------------------------------------------------
   nv_notify, ps_src[i], type=0, desc='ALL'
   nv_notify, /flush
  end
