...
return, nv_copy, ddp_dst, ddp_src, noevent=noevent
@nv.include
 nv_notify, ddp_src, typ=1, desc='ALL', noevent=noevent
...
; shouldn't this be checking update on the destination descriptor?
 ...
 if(NOT keyword_set(update)) then $
  begin
   ...
   if(ptr_valid(dd.dim_p)) then new_dd.dim_p = nv_ptr_new(*dd.dim_p)
   ...
   cor_copy_descriptor, new_dd.crd, nv_core(ddp_src)
 ...
 ;---------------------------------------------------------
 nv_notify, ddp_old, type=0, desc='ALL', noevent=noevent
