...
;-
;=============================================================================
pro dat_set_gd, xd, gd, _extra=gdx
 ...
 ;--------------------------------------------------
 _dd = cor_gd(xds, /dd)
 if(NOT keyword_set(dd)) then for i=0, nxd-1 do cor_set_gd, xd[i], xds=xds $
 else for i=0, nxd-1 do $
   begin
    w = where(_dd EQ dd[i])
    if(w[0] NE -1) then cor_set_gd, xd[i], xds=xds[w]
   end
