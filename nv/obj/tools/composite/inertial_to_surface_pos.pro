...
;+
; NAME:
;       inertial_to_surface_pos
;
;
; PURPOSE:
;       Transforms position vectors in inertial coordinates to surface 
;       coordinates.
;
;
; CATEGORY:
...
;
; CALLING SEQUENCE:
;       result = inertial_to_surface_pos(bx, inertial_pts)
;
;
; ARGUMENTS:
;  INPUT:
...
;
;	inertial_pts:   Array (nv x 3 x nt) of inertial position vectors.
;
;  OUTPUT:
;       NONE
;
...
;-
;=============================================================================
function inertial_to_surface_pos, bx, p
 ...
 if(keyword_set(gbx)) then $
            result[*,*,ii_gbx] = inertial_to_globe_pos(gbx, p[*,*,ii_gbx])
 if(keyword_set(dkx)) then $
            result[*,*,ii_dkx] = inertial_to_disk_pos(dkx, p[*,*,ii_dkx])
