...
;-
;=============================================================================
function dsk_get_outer_disk_points, dkxp, n_points, dlon=dlon, frame_bd=frame_bd, $
              disk_pts=r_disk
@nv_lib.include
 ...
 ;----------------------------------------
 if(NOT keyword_set(dlon)) then dlon = dindgen(n_points)*2d*!dpi/float(n_points)
 ...
 r_disk[*,0,*] = dsk_get_edge_radius(dkdp, dlon, frame_bd, /outer)
 r_disk[*,1,*] = dlon

 ;-------------------------------------
 ; get elevations
 ;-------------------------------------
 r_disk[*,2,*] = dsk_get_edge_elevation(dkdp, dlon, frame_bd, /outer)
...
;===========================================================================
; dsk_get_outer_disk_points.pro
;
; Outputs are in disk body coordinates.
;
;===========================================================================
function _dsk_get_outer_disk_points, dkdp, n_points, dlon=dlon
 ...
 ; disk longitude of each point
 ;----------------------------------------
 if(NOT keyword__set(dlon)) then dlon = dindgen(n_points)*2d*!dpi/float(n_points)
 ...
 r_disk[*,0,*] = dsk_get_outer_radius(dkdp, r_disk[*,1,*])
