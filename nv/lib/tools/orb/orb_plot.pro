...
pro orb_plot, cd, _rd, frame_bx
 ...
 frame_bd = class_extract(frame_bx, 'BODY')
 rd = nv_clone(_rd)
 bod_set_pos, rd, bod_pos(frame_bd)
 ...
 plot_inertial, cd, origins, Q*bod_orient(frame_bd), $
                                  col=ctyellow(), th=1, labels=['0','1','2']

 iframe_bd = orb_inertialize(frame_bd)
 plot_inertial, cd, origins, Q*bod_orient(iframe_bd), $
 ...
 zz = (bod_orient(frame_bd))[2,*] ## make_array(n,val=1d)
 ...
 plot_inertial, cd, origin, $
                        Q*orb_get_ascending_node(rd, frame_bd), col=ctpurple()
